...
export interface LessonTableBlock {
  type: "table";
  header?: LessonTableCell[];
  rows?: { cells?: LessonTableCell[] }[];
}
      ...
      {rows.map((row, rowIndex) => (
        ...
        >
          {(row.cells ?? []).map((cell, cellIndex) => (
            <View key={`c-${cellIndex}`} style={styles.tableCell}>
...
const styles = StyleSheet.create({
  ...
  tableHeadRow: { backgroundColor: "#eef3fb", borderTopWidth: 0 },
  ...
  tableCell: {
    ...
    paddingVertical: 9,
    borderLeftColor: "#e3e8f0",
    borderLeftWidth: 1,
  },
  tableHeadText: { color: "#25304a", fontSize: 14, fontWeight: "800", lineHeight: 20 },
  tableText: { color: "#333a48", fontSize: 14, lineHeight: 20 },
  formulaText: {
    color: colors.text,
    fontSize: 20,
    lineHeight: 28,
