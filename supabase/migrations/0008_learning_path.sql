-- 0008 — учебный путь: уроки, roadmap, дневные задачи, серия дней.
...
-- Отложенный внешний ключ из 0006.
...
-- Перепланирование создаёт новую версию, старая остаётся в истории.
-- Активной по предмету может быть только одна.
...
create table public.daily_plans (
  ...
  -- Идемпотентность генерации: два устройства, открывшие приложение
  -- одновременно, получат один и тот же план, а не два разных.
...
create table public.daily_plan_items (
  ...
  -- Пункт обязан куда-то вести: пустая карточка на дашборде бесполезна.
