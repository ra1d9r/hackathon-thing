  ...
  if (profile.role === 'student') {
    const [row] = await sql<StudentRow[]>`
      select
        ...
        -- Отправленная диагностика считается пройденной, даже если разбор
        -- в очереди ещё идёт или окончательно провалился. Иначе отказ модели
        -- запирал бы ученика в петле «табы → тест → результат → табы»: флаг
        -- ставит только успешный разбор, и снять его было бы некому.
        (coalesce(sp.passed_diagnostics, false) or exists (
           select 1 from public.attempts a
             join public.assessments asm on asm.id = a.assessment_id
            where a.student_id = p.id and asm.kind = 'diagnostic'
              and a.status in ('submitted','grading','graded','failed')
         ))                                          as passed_diagnostics,
...
export async function prepareAvatarUpload(
  ...
  // Имя файла пользователя в путь не попадает: обход каталогов невозможен,
  // а первый сегмент — идентификатор владельца, на нём держится политика Storage.
...
export async function commitAvatar(
  ...
  // Заявленный тип не значит ничего: проверяем сигнатуру самого содержимого.
